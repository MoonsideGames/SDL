...
#include "SDL_GpuDriver_Vulkan_vkfuncs.h"
...
#define WINDOW_DATA "SDL_GpuVulkanWindowData"
 ...
 *
 * Cast from SDL_GpuGpuBuffer or SDL_GpuTransferBuffer.
 */
...
#define VULKAN_ERROR_CHECK(res, fn, ret) \
	if (res != VK_SUCCESS) \
	{ \
		SDL_GpuLogError("%s %s", #fn, VkErrorMessages(res)); \
		return ret; \
	}
...
static inline uint8_t IsRefreshDepthFormat(SDL_GpuTextureFormat format)
{
	switch (format)
	{
		case SDL_GPU_TEXTUREFORMAT_D16_UNORM:
...
static uint8_t VULKAN_INTERNAL_BindResourceMemory(
	...
	{
		SDL_GpuLogError("BindResourceMemory must be given either a VulkanBuffer or a VulkanTexture");
		return 0;
	}
...
static uint8_t VULKAN_INTERNAL_BindMemoryForImage(
	...
	if (bindResult != 1)
	{
		...
		SDL_GpuLogWarn("Out of device-local memory, allocating textures on host-local memory!");
...
static uint8_t VULKAN_INTERNAL_BindMemoryForBuffer(
	...
	if (bindResult != 1)
	{
		...
		{
			SDL_GpuLogInfo("Integrated memory detected, allocating TransferBuffers on device-local memory!");
			renderer->integratedMemoryNotification = 1;
		}

		if (preferDeviceLocal && !renderer->outOfDeviceLocalMemoryWarning)
		{
			SDL_GpuLogWarn("Out of device-local memory, allocating GpuBuffers on host-local memory, expect degraded performance!");
...
static VkDescriptorSetLayout VULKAN_INTERNAL_FetchDescriptorSetLayout(
	...
	if (bindingCount == 0)
	{
		...
		else if (shaderStageFlagBit == VK_SHADER_STAGE_COMPUTE_BIT)
		{
			...
			{
				SDL_GpuLogError("Invalid descriptor type for compute shader: ", descriptorType);
				return NULL_DESC_LAYOUT;
			}
		}
		else
		{
			SDL_GpuLogError("Invalid shader stage flag bit: ", shaderStageFlagBit);
			return NULL_DESC_LAYOUT;
		}
	}
...
static VulkanBufferHandle* VULKAN_INTERNAL_CreateBufferHandle(
	...
	if (buffer == NULL)
	{
		SDL_GpuLogError("Failed to create buffer!");
		return NULL;
	}
...
static VulkanBufferContainer* VULKAN_INTERNAL_CreateBufferContainer(
	...
	{
		SDL_GpuLogError("Failed to create buffer container!");
		return NULL;
	}
...
static uint8_t VULKAN_INTERNAL_AddUniformDescriptorPool(
	...
	if (!VULKAN_INTERNAL_CreateDescriptorPool(
		...
		SDL_GpuLogError("Failed to create descriptor pool!");
		return 0;
	}
...
static uint8_t VULKAN_INTERNAL_CreateUniformBuffer(
	...
	{
		SDL_GpuLogError("Unrecognized uniform buffer type!");
		return 0;
	}
	...
	{
		SDL_GpuLogError("Failed to create uniform buffer!");
	...
	if (bufferPool->descriptorPool.availableDescriptorSetCount == 0)
	{
		if (!VULKAN_INTERNAL_AddUniformDescriptorPool(
			...
			SDL_GpuLogError("Failed to add uniform descriptor pool!");
			SDL_free(uniformBuffer);
			return 0;
		}
	}
	...
	if (!VULKAN_INTERNAL_AllocateDescriptorSets(
		...
		SDL_GpuLogError("Failed to allocate uniform descriptor set!");
		return 0;
	}
...
static void VULKAN_INTERNAL_CreateSliceView(
	...
	if (vulkanResult != VK_SUCCESS)
	{
		...
		SDL_GpuLogError("Failed to create color attachment image view");
		*pView = NULL;
		return;
	}
}
...
static uint8_t VULKAN_INTERNAL_QuerySwapChainSupport(
	...
	if (!supportsPresent)
	{
		SDL_GpuLogWarn("This surface does not support presenting!");
		return 0;
	}
	...
	{
		SDL_GpuLogWarn("Opaque presentation unsupported! Expect weird transparency bugs!");
	}
...
static uint8_t VULKAN_INTERNAL_ChooseSwapPresentMode(
	SDL_GpuPresentMode desiredPresentInterval,
	...
	uint32_t i;
	if (desiredPresentInterval == SDL_GPU_PRESENTMODE_IMMEDIATE)
	{
		CHECK_MODE(VK_PRESENT_MODE_IMMEDIATE_KHR)
	}
	else if (desiredPresentInterval == SDL_GPU_PRESENTMODE_MAILBOX)
	{
		CHECK_MODE(VK_PRESENT_MODE_MAILBOX_KHR)
	}
	else if (desiredPresentInterval == SDL_GPU_PRESENTMODE_FIFO)
	{
		CHECK_MODE(VK_PRESENT_MODE_FIFO_KHR)
	}
	else if (desiredPresentInterval == SDL_GPU_PRESENTMODE_FIFO_RELAXED)
	{
...
static uint8_t VULKAN_INTERNAL_CreateSwapchain(
	...
	if (!VULKAN_INTERNAL_QuerySwapChainSupport(
		...
		SDL_free(swapchainData);
		SDL_GpuLogError("Device does not support swap chain creation");
		return 0;
	}
	...
	if (!VULKAN_INTERNAL_ChooseSwapSurfaceFormat(
		...
		if (!VULKAN_INTERNAL_ChooseSwapSurfaceFormat(
			...
			SDL_free(swapchainData);
			SDL_GpuLogError("Device does not support swap chain format");
			return 0;
		}
	}
	...
	if (!VULKAN_INTERNAL_ChooseSwapPresentMode(
		...
		SDL_free(swapchainData);
		SDL_GpuLogError("Device does not support swap chain present mode");
		return 0;
	}
		...
		drawableHeight > swapchainSupportDetails.capabilities.maxImageExtent.height	)
	{
		...
		else
		{
			...
			SDL_free(swapchainData);
			SDL_GpuLogError("No fallback swapchain size available!");
			return 0;
		}
	}
...
static VulkanTexture* VULKAN_INTERNAL_CreateTexture(
	...
	if (bindResult != 1)
	{
		...
		SDL_GpuLogError("Unable to bind memory for texture!");
		return NULL;
	}
	...
	{
		LogVulkanResultAsError("vkCreateImageView", vulkanResult);
		SDL_GpuLogError("Failed to create texture image view");
		return NULL;
	}
...
static VulkanTextureHandle* VULKAN_INTERNAL_CreateTextureHandle(
	...
	if (texture == NULL)
	{
		SDL_GpuLogError("Failed to create texture!");
		return NULL;
	}
...
static SDL_GpuGraphicsPipeline* VULKAN_CreateGraphicsPipeline(
	...
	if (vulkanResult != VK_SUCCESS)
	{
		...
		SDL_GpuLogError("Failed to create graphics pipeline!");
		return NULL;
	}
...
static SDL_GpuShaderModule* VULKAN_CreateShaderModule(
	...
	if (vulkanResult != VK_SUCCESS)
	{
		...
		SDL_GpuLogError("Failed to create shader module!");
		return NULL;
	}
...
static SDL_GpuTexture* VULKAN_CreateTexture(
	...
	{
		SDL_GpuLogInfo("Failed to create texture container!");
		return NULL;
	}
...
static SDL_GpuGpuBuffer* VULKAN_CreateGpuBuffer(
	SDL_GpuRenderer *driverData,
	...
	return (SDL_GpuGpuBuffer*) VULKAN_INTERNAL_CreateBufferContainer(
...
static VkDescriptorSet VULKAN_INTERNAL_FetchDescriptorSet(
	...
	{
		SDL_GpuLogError("descriptorImageInfos and descriptorBufferInfos cannot both be NULL!");
		return VK_NULL_HANDLE;
	}
	else if (descriptorImageInfos != NULL && descriptorBufferInfos != NULL)
	{
		SDL_GpuLogError("descriptorImageInfos and descriptorBufferInfos cannot both be set!");
		return VK_NULL_HANDLE;
	}
	...
	if (descriptorSetCache->inactiveDescriptorSetCount == 0)
	{
		...
		if (!VULKAN_INTERNAL_AllocateDescriptorSets(
			...
			SDL_UnlockMutex(descriptorSetCache->lock);
			SDL_GpuLogError("Failed to allocate descriptor sets!");
			return VK_NULL_HANDLE;
		}
...
static void VULKAN_BeginRenderPass(
	...
	for (i = 0; i < colorAttachmentCount; i += 1)
	{
		...
		{
			SDL_GpuLogError("Color attachment texture was not designated as a target!");
			return;
		}
	}
	...
	if (depthStencilAttachmentInfo != NULL)
	{
		...
		if (!textureContainer->activeTextureHandle->vulkanTexture->isRenderTarget)
		{
			SDL_GpuLogError("Depth stencil attachment texture was not designated as a target!");
			return;
		}
	}
...
static VulkanCommandPool* VULKAN_INTERNAL_FetchCommandPool(
	...
	if (vulkanResult != VK_SUCCESS)
	{
		SDL_GpuLogError("Failed to create command pool!");
...
static uint8_t VULKAN_ClaimWindow(
	...
	if (windowData == NULL)
	{
		...
		{
			SDL_GpuLogError("Could not create swapchain, failed to claim window!");
			SDL_free(windowData);
			return 0;
		}
	}
	...
	{
		SDL_GpuLogWarn("Window already claimed!");
		return 0;
	}
}
...
static SDL_GpuTexture* VULKAN_AcquireSwapchainTexture(
	...
	if (swapchainData == NULL)
	{
		...
		if (swapchainData == NULL)
		{
			SDL_GpuLogWarn("Failed to recreate swapchain!");
			return NULL;
		}
	}
	...
	if (acquireResult != VK_SUCCESS && acquireResult != VK_SUBOPTIMAL_KHR)
	{
		...
		if (acquireResult != VK_SUCCESS && acquireResult != VK_SUBOPTIMAL_KHR)
		{
			SDL_GpuLogWarn("Failed to acquire swapchain texture!");
			return NULL;
		}
	}
...
static SDL_GpuTextureFormat VULKAN_GetSwapchainFormat(
	...
	if (windowData == NULL)
	{
		SDL_GpuLogWarn("Cannot get swapchain format, window has not been claimed!");
		return 0;
	}

	if (windowData->swapchainData == NULL)
	{
		SDL_GpuLogWarn("Cannot get swapchain format, swapchain is currently invalid!");
		return 0;
	}
	...
	{
		SDL_GpuLogWarn("Unrecognized swapchain format!");
		return 0;
	}
}
...
static void VULKAN_SetSwapchainPresentMode(
	...
	if (windowData == NULL)
	{
		SDL_GpuLogWarn("Cannot set present mode, window has not been claimed!");
		return;
	}
...
static uint8_t VULKAN_INTERNAL_DefragmentMemory(
	...
	for (i = 0; i < allocation->usedRegionCount; i += 1)
	{
		...
		if (currentRegion->isBuffer && !currentRegion->vulkanBuffer->markedForDestroy)
		{
			...
			if (newBuffer == NULL)
			{
				SDL_GpuLogError("Failed to create defrag buffer!");
				return 0;
			}
		...
		else if (!currentRegion->vulkanTexture->markedForDestroy)
		{
			...
			if (newTexture == NULL)
			{
				SDL_GpuLogError("Failed to create defrag texture!");
				return 0;
			}
...
static uint8_t VULKAN_INTERNAL_CreateInstance(
	...
	appInfo.engineVersion = SDL_GPU_COMPILED_VERSION;
	...
	if (renderer->debugMode)
	{
		...
		if (!VULKAN_INTERNAL_CheckValidationLayers(
			...
			SDL_GpuLogWarn("Validation layers not found, continuing without validation");
			createInfo.enabledLayerCount = 0;
		}
		else
		{
			SDL_GpuLogInfo("Validation layers enabled, expect debug level performance!");
		}
	}
	else
	{
...
static uint8_t VULKAN_INTERNAL_DeterminePhysicalDevice(
	...
	{
		SDL_GpuLogWarn("Failed to find any GPUs with Vulkan support");
		return 0;
	}
	...
	{
		SDL_GpuLogWarn("vkEnumeratePhysicalDevices returned VK_INCOMPLETE, will keep trying anyway...");
...
static void VULKAN_INTERNAL_LoadEntryPoints(void)
{
	...
	{
		SDL_GpuLogWarn("Vulkan: SDL_Vulkan_LoadLibrary failed!");
		return;
	}
	...
	#define VULKAN_GLOBAL_FUNCTION(name)								\
			...
			if (name == NULL)									\
			{											\
				SDL_GpuLogWarn("vkGetInstanceProcAddr(VK_NULL_HANDLE, \"" #name "\") failed");	\
				return;									\
			}
	#include "SDL_GpuDriver_Vulkan_vkfuncs.h"
}
...
static uint8_t VULKAN_INTERNAL_PrepareVulkan(
	...
	{
		SDL_GpuLogWarn("Vulkan: Could not create dummy window");
		return 0;
	}
	...
	if (!VULKAN_INTERNAL_CreateInstance(renderer, dummyWindowHandle))
	{
		...
		SDL_free(renderer);
		SDL_GpuLogWarn("Vulkan: Could not create Vulkan instance");
		return 0;
	}
...
static uint8_t VULKAN_PrepareDriver(uint32_t *flags)
{
	...
	if (!result)
	{
		SDL_GpuLogWarn("Vulkan: Failed to determine a suitable physical device");
	}
	else
	{
...
static SDL_GpuDevice* VULKAN_CreateDevice(
	...
	{
		SDL_GpuLogError("Failed to initialize Vulkan!");
		return NULL;
	}

	SDL_GpuLogInfo("SDL_Gpu Driver: Vulkan");
	SDL_GpuLogInfo(
		"Vulkan Device: %s",
	...
	if (!VULKAN_INTERNAL_CreateLogicalDevice(
		...
		SDL_GpuLogError("Failed to create logical device");
		return NULL;
	}
	...
	if (vulkanResult != VK_SUCCESS)
	{
		SDL_GpuLogError("Failed to create vertex UBO layout!");
		return NULL;
	}
	...
	if (vulkanResult != VK_SUCCESS)
	{
		SDL_GpuLogError("Failed to create fragment UBO layout!");
		return NULL;
	}
