...
/* FIXME: some of these are unused, naming pattern is inconsistent */
typedef enum VulkanResourceAccessType
{
    ...
    RESOURCE_ACCESS_GRAPHICS_SHADER_WRITE_STORAGE_IMAGE,
    ...
    RESOURCE_ACCESS_GRAPHICS_SHADER_STORAGE_IMAGE_READ_WRITE,
...
static const VulkanResourceAccessInfo AccessMap[RESOURCE_ACCESS_TYPES_COUNT] =
{
    ...
    /* RESOURCE_ACCESS_GRAPHICS_SHADER_STORAGE_IMAGE_WRITE */
    {
        VK_PIPELINE_STAGE_VERTEX_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
        VK_ACCESS_SHADER_WRITE_BIT,
        VK_IMAGE_LAYOUT_GENERAL
...
typedef struct VulkanGraphicsPipelineLayout
{
    ...
    DescriptorSetCache *vertexStorageBufferDescriptorSetCache;
    DescriptorSetCache *fragmentStorageBufferDescriptorSetCache;
...
typedef struct GraphicsPipelineLayoutHash
{
    ...
    VkDescriptorSetLayout vertexStorageBufferLayout;
    VkDescriptorSetLayout fragmentStorageBufferLayout;
...
static inline uint64_t GraphicsPipelineLayoutHashTable_GetHashCode(GraphicsPipelineLayoutHash key)
{
    ...
    result = result * HASH_FACTOR + (uint64_t) key.vertexStorageBufferLayout;
    result = result * HASH_FACTOR + (uint64_t) key.fragmentStorageBufferLayout;
...
static inline VulkanGraphicsPipelineLayout* GraphicsPipelineLayoutHashArray_Fetch(
    ...
    for (i = 0; i < arr->count; i += 1)
    {
        ...
        if (	key.vertexSamplerLayout == e->vertexSamplerLayout &&
            ...
            key.vertexStorageBufferLayout == e->vertexStorageBufferLayout &&
            key.fragmentStorageBufferLayout == e->fragmentStorageBufferLayout &&
...
typedef struct VulkanCommandBuffer
{
    ...
    /* Keep track of compute pass resources for memory barriers */

    VulkanBuffer **computePassBuffers;
    Uint32 computePassBufferCount;
    Uint32 computePassBufferCapacity;

    VulkanTextureSlice **computePassTextureSlices;
    Uint32 computePassTextureSliceCount;
    Uint32 computePassTextureSliceCapacity;

    /* Keep track of copy pass resources for memory barriers */

    VulkanBuffer **copyPassBuffers;
    Uint32 copyPassBufferCount;
    Uint32 copyPassBufferCapacity;

    VulkanTextureSlice **copyPassTextureSlices;
    Uint32 copyPassTextureSliceCount;
    Uint32 copyPassTextureSliceCapacity;

    /* Keep track of render pass resources for memory barriers */

    VulkanBuffer **renderPassStorageBuffers;
    Uint32 renderPassStorageBufferCount;
    Uint32 renderPassStorageBufferCapacity;

    VulkanTextureSlice **renderPassStorageTextureSlices;
    Uint32 renderPassStorageTextureSliceCount;
    Uint32 renderPassStorageTextureSliceCapacity;
...
struct VulkanRenderer
{
    ...
    VkDescriptorSetLayout emptyVertexStorageBufferLayout;
    VkDescriptorSetLayout emptyFragmentStorageBufferLayout;
    ...
    VkDescriptorSet emptyVertexStorageBufferDescriptorSet;
    VkDescriptorSet emptyFragmentStorageBufferDescriptorSet;
...
static void VULKAN_INTERNAL_DefaultBufferAccessInfo(
...
static void VULKAN_INTERNAL_DefaultTextureSliceAccessInfo(
    SDL_GpuTextureUsageFlags usageFlags,
    VulkanResourceAccessInfo *accessInfo
) {
    VkPipelineStageFlags stageMask = 0;
...
static void VULKAN_INTERNAL_PostPassBufferBarrier(
    ...
    if (
        nextAccessInfo.stageMask == 0 &&
        nextAccessInfo.accessMask == 0
    ) {
        /* No need to transition, bail */
        return;
    }
...
static void VULKAN_INTERNAL_PostPassTextureSliceBarrier(
    ...
    if (
        nextAccessInfo.stageMask == 0 &&
        nextAccessInfo.accessMask == 0 &&
        nextAccessInfo.imageLayout == VK_IMAGE_LAYOUT_UNDEFINED
    ) {
        /* No need to transition, bail */
        return;
    }
...
static void VULKAN_INTERNAL_TrackComputePassBuffer(
    VulkanRenderer *renderer,
    VulkanCommandBuffer *commandBuffer,
    ...
    ADD_TO_ARRAY_UNIQUE(
        ...
        VulkanBuffer*,
        computePassBuffers,
        computePassBufferCount,
        computePassBufferCapacity
    );
}

static void VULKAN_INTERNAL_TrackComputePassTextureSlice(
    VulkanRenderer *renderer,
    VulkanCommandBuffer *commandBuffer,
    VulkanTextureSlice *textureSlice
) {
    ADD_TO_ARRAY_UNIQUE(
        textureSlice,
        VulkanTextureSlice*,
        computePassTextureSlices,
        computePassTextureSliceCount,
        computePassTextureSliceCapacity
    );
}

/* For tracking Textures used in a copy pass. */
static void VULKAN_INTERNAL_TrackCopyPassTextureSlice(
    VulkanRenderer *renderer,
    VulkanCommandBuffer *commandBuffer,
    VulkanTextureSlice *textureSlice
) {
    ADD_TO_ARRAY_UNIQUE(
        textureSlice,
        VulkanTextureSlice*,
        copyPassTextureSlices,
        copyPassTextureSliceCount,
        copyPassTextureSliceCapacity
    );
}

/* For tracking GpuBuffers used in a copy pass. */
static void VULKAN_INTERNAL_TrackCopyPassBuffer(
    VulkanRenderer *renderer,
    VulkanCommandBuffer *commandBuffer,
    VulkanBuffer *buffer
) {
    ADD_TO_ARRAY_UNIQUE(
        ...
        copyPassBuffers,
        copyPassBufferCount,
        copyPassBufferCapacity
    );
}

static void VULKAN_INTERNAL_TrackRenderPassStorageTextureSlice(
    VulkanRenderer *renderer,
    VulkanCommandBuffer *commandBuffer,
    VulkanTextureSlice* textureSlice
) {
    ADD_TO_ARRAY_UNIQUE(
        textureSlice,
        VulkanTextureSlice*,
        renderPassStorageTextureSlices,
        renderPassStorageTextureSliceCount,
        renderPassStorageTextureSliceCapacity
    );
}

static void VULKAN_INTERNAL_TrackRenderPassStorageBuffer(
    VulkanRenderer *renderer,
    VulkanCommandBuffer *commandBuffer,
    VulkanBuffer *buffer
) {
    ADD_TO_ARRAY_UNIQUE(
        buffer,
        VulkanBuffer*,
        renderPassStorageBuffers,
        renderPassStorageBufferCount,
        renderPassStorageBufferCapacity
    );
}
...
static void VULKAN_INTERNAL_DestroyCommandPool(
    ...
    for (i = 0; i < commandPool->inactiveCommandBufferCount; i += 1)
    {
        ...
        SDL_free(commandBuffer->computePassBuffers);
        SDL_free(commandBuffer->computePassTextureSlices);
        SDL_free(commandBuffer->copyPassBuffers);
        SDL_free(commandBuffer->copyPassTextureSlices);
        SDL_free(commandBuffer->renderPassStorageBuffers);
        SDL_free(commandBuffer->renderPassStorageTextureSlices);
...
static VkDescriptorSetLayout VULKAN_INTERNAL_FetchDescriptorSetLayout(
    ...
    if (bindingCount == 0)
    {
        if (shaderStageFlagBit == VK_SHADER_STAGE_VERTEX_BIT)
        {
            ...
            {
                return renderer->emptyVertexStorageBufferLayout;
            }
            else
            {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Invalid descriptor type for vertex shader: %i", descriptorType);
        ...
        else if (shaderStageFlagBit == VK_SHADER_STAGE_FRAGMENT_BIT)
        {
            ...
            {
                return renderer->emptyFragmentStorageBufferLayout;
            }
            else
            {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Invalid descriptor type for fragment shader: %i", descriptorType);
                return NULL_DESC_LAYOUT;
            }
        }
...
static VulkanGraphicsPipelineLayout* VULKAN_INTERNAL_FetchGraphicsPipelineLayout(
    ...
    Uint32 vertexStorageBindingCount,
    Uint32 fragmentStorageBindingCount
) {
    ...
    pipelineLayoutHash.vertexStorageBufferLayout = VULKAN_INTERNAL_FetchDescriptorSetLayout(
        renderer,
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
        vertexStorageBindingCount,
        VK_SHADER_STAGE_VERTEX_BIT
    );

    pipelineLayoutHash.fragmentStorageBufferLayout = VULKAN_INTERNAL_FetchDescriptorSetLayout(
        renderer,
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
        fragmentStorageBindingCount,
        VK_SHADER_STAGE_FRAGMENT_BIT
    );
    ...
    setLayouts[4] = pipelineLayoutHash.vertexStorageBufferLayout;
    setLayouts[5] = pipelineLayoutHash.fragmentStorageBufferLayout;

    pipelineLayoutCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    pipelineLayoutCreateInfo.pNext = NULL;
    pipelineLayoutCreateInfo.flags = 0;
    pipelineLayoutCreateInfo.setLayoutCount = 6;
    ...
    if (vertexStorageBindingCount == 0)
    {
        vulkanGraphicsPipelineLayout->vertexStorageBufferDescriptorSetCache = NULL;
    }
    else
    {
        vulkanGraphicsPipelineLayout->vertexStorageBufferDescriptorSetCache =
            VULKAN_INTERNAL_CreateDescriptorSetCache(
                renderer,
                VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                pipelineLayoutHash.vertexStorageBufferLayout,
                vertexSamplerBindingCount
            );
    }

    if (fragmentStorageBindingCount == 0)
    {
        vulkanGraphicsPipelineLayout->fragmentStorageBufferDescriptorSetCache = NULL;
    }
    else
    {
        vulkanGraphicsPipelineLayout->fragmentStorageBufferDescriptorSetCache =
            VULKAN_INTERNAL_CreateDescriptorSetCache(
                renderer,
                VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                pipelineLayoutHash.fragmentStorageBufferLayout,
                fragmentSamplerBindingCount
            );
    }
...
static void VULKAN_DestroyDevice(
    ...
    for (i = 0; i < NUM_PIPELINE_LAYOUT_BUCKETS; i += 1)
    {
        ...
        for (j = 0; j < graphicsPipelineLayoutHashArray.count; j += 1)
        {
            ...
            VULKAN_INTERNAL_DestroyDescriptorSetCache(
                renderer,
                graphicsPipelineLayoutHashArray.elements[j].value->vertexStorageBufferDescriptorSetCache
            );

            VULKAN_INTERNAL_DestroyDescriptorSetCache(
                renderer,
                graphicsPipelineLayoutHashArray.elements[j].value->fragmentStorageBufferDescriptorSetCache
            );
...
static void VULKAN_INTERNAL_BindGraphicsDescriptorSets(
    ...
    descriptorSets[4] = vulkanCommandBuffer->vertexStorageDescriptorSet;
    descriptorSets[5] = vulkanCommandBuffer->fragmentStorageDescriptorSet;
...
static VulkanTexture* VULKAN_INTERNAL_CreateTexture(
    ...
    if (imageUsageFlags & SDL_GPU_TEXTUREUSAGE_SAMPLER_BIT)
    {
        vkUsageFlags = VK_IMAGE_USAGE_SAMPLED_BIT;
    }
    if (imageUsageFlags & SDL_GPU_TEXTUREUSAGE_COLOR_TARGET_BIT)
    {
        vkUsageFlags = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
    }
    if (imageUsageFlags & SDL_GPU_TEXTUREUSAGE_DEPTH_STENCIL_TARGET_BIT)
    {
...
static SDL_GpuShaderModule* VULKAN_CreateShaderModule(
    ...
    if (shaderModuleCreateInfo->format != SDL_GPU_SHADERFORMAT_SPIRV)
	{
...
static SDL_GpuTexture* VULKAN_CreateTexture(
    ...
    if (textureCreateInfo->usageFlags & (
            SDL_GPU_TEXTUREUSAGE_GRAPHICS_STORAGE_READ_BIT |
            SDL_GPU_TEXTUREUSAGE_GRAPHICS_STORAGE_WRITE_BIT |
            SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_READ_BIT |
            SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_WRITE_BIT
...
static void VULKAN_BindGraphicsPipeline(
    ...
    if (pipeline->pipelineLayout->vertexStorageBufferDescriptorSetCache == NULL)
    {
        vulkanCommandBuffer->vertexStorageDescriptorSet = renderer->emptyVertexStorageBufferDescriptorSet;
    }

    if (pipeline->pipelineLayout->fragmentStorageBufferDescriptorSetCache == NULL)
    {
        vulkanCommandBuffer->fragmentStorageDescriptorSet = renderer->emptyFragmentStorageBufferDescriptorSet;
    }
...
static void VULKAN_BindVertexStorageBuffers(
    ...
    if (graphicsPipeline->pipelineLayout->vertexStorageBufferDescriptorSetCache == NULL)
    {
        return;
    }

    storageBufferCount = graphicsPipeline->pipelineLayout->vertexStorageBufferDescriptorSetCache->bindingCount;
    ...
    for (i = 0; i < storageBufferCount; i += 1)
    {
        ...
        VULKAN_INTERNAL_TrackRenderPassStorageBuffer(
            renderer,
            vulkanCommandBuffer,
            currentVulkanBuffer
        );
    ...
    vulkanCommandBuffer->vertexStorageDescriptorSet =
        VULKAN_INTERNAL_FetchDescriptorSet(
            renderer,
            vulkanCommandBuffer,
            graphicsPipeline->pipelineLayout->vertexStorageBufferDescriptorSetCache,
            NULL,
...
static void VULKAN_BindFragmentStorageBuffers(
    ...
    if (graphicsPipeline->pipelineLayout->fragmentStorageBufferDescriptorSetCache == NULL)
    {
        return;
    }

    storageBufferCount = graphicsPipeline->pipelineLayout->fragmentStorageBufferDescriptorSetCache->bindingCount;
    ...
    vulkanCommandBuffer->fragmentStorageDescriptorSet =
        VULKAN_INTERNAL_FetchDescriptorSet(
            renderer,
            vulkanCommandBuffer,
            graphicsPipeline->pipelineLayout->fragmentStorageBufferDescriptorSetCache,
            NULL,
            descriptorBufferInfos
        );
}
...
static void VULKAN_EndRenderPass(
    ...
    /* FIXME: actually track these */
    for (i = 0; i < vulkanCommandBuffer->renderPassStorageTextureSliceCount; i += 1)
    {
        VULKAN_INTERNAL_PostPassBufferBarrier(
            renderer,
            vulkanCommandBuffer,
            vulkanCommandBuffer->renderPassStorageBuffers[i]
        );
    }
    vulkanCommandBuffer->renderPassStorageTextureSliceCount = 0;
...
static void VULKAN_BeginComputePass(
    ...
    vulkanCommandBuffer->computePassBufferCount = 0;
    vulkanCommandBuffer->computePassTextureSliceCount = 0;
}
...
static void VULKAN_BindComputePipeline(
    ...
    {
        vulkanCommandBuffer->computeBufferDescriptorSet = renderer->emptyComputeBufferDescriptorSet;
    }

    if (vulkanComputePipeline->pipelineLayout->imageDescriptorSetCache == NULL)
    {
        vulkanCommandBuffer->computeImageDescriptorSet = renderer->emptyComputeImageDescriptorSet;
    }
...
static void VULKAN_BindComputeStorageBuffers(
    ...
    for (i = 0; i < computePipeline->pipelineLayout->bufferDescriptorSetCache->bindingCount; i += 1)
    {
        ...
        VULKAN_INTERNAL_TrackBuffer(renderer, vulkanCommandBuffer, currentVulkanBuffer);
        VULKAN_INTERNAL_TrackComputePassBuffer(renderer, vulkanCommandBuffer, currentVulkanBuffer);
    }

    vulkanCommandBuffer->computeBufferDescriptorSet =
        VULKAN_INTERNAL_FetchDescriptorSet(
            renderer,
            vulkanCommandBuffer,
...
static void VULKAN_BindComputeTextures(
    ...
    for (i = 0; i < computePipeline->pipelineLayout->imageDescriptorSetCache->bindingCount; i += 1)
    {
        ...
        VULKAN_INTERNAL_TrackComputePassTextureSlice(renderer, vulkanCommandBuffer, currentTextureSlice);
    }

    vulkanCommandBuffer->computeImageDescriptorSet =
        VULKAN_INTERNAL_FetchDescriptorSet(
            renderer,
            vulkanCommandBuffer,
...
static void VULKAN_DispatchCompute(
    ...
    descriptorSets[0] = vulkanCommandBuffer->computeBufferDescriptorSet;
    descriptorSets[1] = vulkanCommandBuffer->computeImageDescriptorSet;
...
static void VULKAN_EndComputePass(
    ...
    for (i = 0; i < vulkanCommandBuffer->computePassBufferCount; i += 1)
    {
        currentComputeBuffer = vulkanCommandBuffer->computePassBuffers[i];
    ...
    for (i = 0; i < vulkanCommandBuffer->computePassTextureSliceCount; i += 1)
    {
        currentComputeTextureSlice = vulkanCommandBuffer->computePassTextureSlices[i];
...
static void VULKAN_BeginCopyPass(
    ...
    vulkanCommandBuffer->copyPassBufferCount = 0;
    vulkanCommandBuffer->copyPassTextureSliceCount = 0;
}
...
static void VULKAN_UploadToTexture(
    ...
    VULKAN_INTERNAL_TrackCopyPassTextureSlice(renderer, vulkanCommandBuffer, vulkanTextureSlice);
}
...
static void VULKAN_UploadToBuffer(
    ...
    VULKAN_INTERNAL_TrackBuffer(renderer, vulkanCommandBuffer, transferBufferContainer->activeBufferHandle->vulkanBuffer);
    VULKAN_INTERNAL_TrackBuffer(renderer, vulkanCommandBuffer, vulkanBuffer);
    VULKAN_INTERNAL_TrackCopyPassBuffer(renderer, vulkanCommandBuffer, vulkanBuffer);
}
...
static void VULKAN_CopyTextureToTexture(
    ...
    VULKAN_INTERNAL_TrackCopyPassTextureSlice(renderer, vulkanCommandBuffer, srcSlice);
    VULKAN_INTERNAL_TrackCopyPassTextureSlice(renderer, vulkanCommandBuffer, dstSlice);
}
...
static void VULKAN_CopyBufferToBuffer(
    ...
    VULKAN_INTERNAL_TrackCopyPassBuffer(renderer, vulkanCommandBuffer, srcContainer->activeBufferHandle->vulkanBuffer);
    VULKAN_INTERNAL_TrackCopyPassBuffer(renderer, vulkanCommandBuffer, dstBuffer);
}
...
static void VULKAN_GenerateMipmaps(
    ...
    for (level = 1; level < vulkanTexture->levelCount; level += 1)
    {
        ...
        VULKAN_INTERNAL_TrackCopyPassTextureSlice(renderer, vulkanCommandBuffer, srcTextureSlice);
        VULKAN_INTERNAL_TrackCopyPassTextureSlice(renderer, vulkanCommandBuffer, dstTextureSlice);
    }
}
...
static void VULKAN_EndCopyPass(
    ...
    for (i = 0; i < vulkanCommandBuffer->copyPassBufferCount; i += 1)
    {
        currentBuffer = vulkanCommandBuffer->copyPassBuffers[i];
    ...
    for (i = 0; i < vulkanCommandBuffer->copyPassTextureSliceCount; i += 1)
    {
        currentTextureSlice = vulkanCommandBuffer->copyPassTextureSlices[i];
...
static void VULKAN_Blit(
    ...
    renderer->vkCmdBlitImage(
        ...
        AccessMap[RESOURCE_ACCESS_TRANSFER_READ].imageLayout,
    ...
    if (dstTextureSlice->parent->usageFlags & VK_IMAGE_USAGE_SAMPLED_BIT)
    {
        VULKAN_INTERNAL_ImageMemoryBarrier(
            renderer,
            vulkanCommandBuffer->commandBuffer,
...
static void VULKAN_INTERNAL_AllocateCommandBuffers(
    ...
    for (i = 0; i < allocateCount; i += 1)
    {
        ...
        /* Compute pass resource tracking */

        commandBuffer->computePassBufferCapacity = 16;
        commandBuffer->computePassBufferCount = 0;
        commandBuffer->computePassBuffers = SDL_malloc(
            commandBuffer->computePassBufferCapacity * sizeof(VulkanBuffer*)
        );

        commandBuffer->computePassTextureSliceCapacity = 16;
        commandBuffer->computePassTextureSliceCount = 0;
        commandBuffer->computePassTextureSlices = SDL_malloc(
            commandBuffer->computePassTextureSliceCapacity * sizeof(VulkanTextureSlice*)
        );

        /* Copy pass resource tracking */

        commandBuffer->copyPassBufferCapacity = 16;
        commandBuffer->copyPassBufferCount = 0;
        commandBuffer->copyPassBuffers = SDL_malloc(
            commandBuffer->copyPassBufferCapacity * sizeof(VulkanBuffer*)
        );

        commandBuffer->copyPassTextureSliceCapacity = 16;
        commandBuffer->copyPassTextureSliceCount = 0;
        commandBuffer->copyPassTextureSlices = SDL_malloc(
            commandBuffer->copyPassTextureSliceCapacity * sizeof(VulkanTextureSlice*)
        );

        /* Render pass resource tracking */

        commandBuffer->renderPassStorageBufferCapacity = 16;
        commandBuffer->renderPassStorageBufferCount = 0;
        commandBuffer->renderPassStorageBuffers = SDL_malloc(
            commandBuffer->renderPassStorageBufferCapacity * sizeof(VulkanBuffer*)
        );

        commandBuffer->renderPassStorageTextureSliceCapacity = 16;
        commandBuffer->renderPassStorageTextureSliceCount = 0;
        commandBuffer->renderPassStorageTextureSlices = SDL_malloc(
            commandBuffer->renderPassStorageTextureSliceCapacity * sizeof(VulkanTextureSlice*)
        );
        ...
        else if (!currentRegion->isBuffer && !currentRegion->vulkanTexture->markedForDestroy)
        {
            ...
            for (sliceIndex = 0; sliceIndex < currentRegion->vulkanTexture->sliceCount; sliceIndex += 1)
            {
                ...
                if (srcSlice->resourceAccessType != RESOURCE_ACCESS_NONE)
                {
                    ...
                    if (dstSlice->parent->usageFlags & VK_IMAGE_USAGE_SAMPLED_BIT)
                    {
                        VULKAN_INTERNAL_ImageMemoryBarrier(
                            renderer,
                            commandBuffer->commandBuffer,
...
static void VULKAN_DownloadFromTexture(
    ...
    if (originalTextureSliceAccessType != RESOURCE_ACCESS_NONE)
    {
        VULKAN_INTERNAL_ImageMemoryBarrier(
            renderer,
            vulkanCommandBuffer->commandBuffer,
...
static void VULKAN_DownloadFromBuffer(
    ...
    if (originalGpuBufferAccessType != RESOURCE_ACCESS_NONE)
    {
        VULKAN_INTERNAL_BufferMemoryBarrier(
            renderer,
            vulkanCommandBuffer->commandBuffer,
...
static SDL_bool VULKAN_IsTextureFormatSupported(
    ...
    if (usage & (SDL_GPU_TEXTUREUSAGE_STORAGE_READ_BIT | SDL_GPU_TEXTUREUSAGE_STORAGE_WRITE_BIT))
    {
...
static SDL_GpuDevice* VULKAN_CreateDevice(
    ...
    vulkanResult = renderer->vkCreateDescriptorSetLayout(
        ...
        &setLayoutCreateInfo,
        NULL,
        &renderer->emptyVertexStorageBufferLayout
    );
    ...
    vulkanResult = renderer->vkCreateDescriptorSetLayout(
        ...
        &setLayoutCreateInfo,
        NULL,
        &renderer->emptyFragmentStorageBufferLayout
    );
    ...
    defaultDescriptorPoolInfo.maxSets = 2 + 3 + 1 + 3;
    ...
    descriptorAllocateInfo.pSetLayouts = &renderer->emptyVertexStorageBufferLayout;

    renderer->vkAllocateDescriptorSets(
        renderer->logicalDevice,
        &descriptorAllocateInfo,
        &renderer->emptyVertexStorageBufferDescriptorSet
    );

    descriptorAllocateInfo.pSetLayouts = &renderer->emptyFragmentStorageBufferLayout;

    renderer->vkAllocateDescriptorSets(
        renderer->logicalDevice,
        &descriptorAllocateInfo,
        &renderer->emptyFragmentStorageBufferDescriptorSet
    );
