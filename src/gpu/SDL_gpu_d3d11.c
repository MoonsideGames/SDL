...
const char* fullscreenVertexShaderCode =
...
const char* blitFrom2DPixelShaderCode =
"struct VertextoPixel"
"{"
    "float4 pos              : SV_POSITION;"
    "float2 tex              : TEXCOORD0;"
...
const char* blitFrom2DArrayPixelShaderCode =
"struct VertextoPixel"
"{"
    "float4 pos              : SV_POSITION;"
    "float2 tex              : TEXCOORD0;"
...
static SDL_GpuShaderModule* D3D11_CreateShaderModule(
	...
	if (FAILED(res))
	{
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "D3DCompile Error: %s\n", (char *) ID3D10Blob_GetBufferPointer(errorBlob));
...
static void D3D11_UploadToTexture(
    ...
    desc2D.ArraySize = 1;
    desc2D.MipLevels = 1;
    desc2D.Format = RefreshToD3D11_TextureFormat[textureSubresource->parent->format];
    desc2D.SampleDesc.Count = 1;
    desc2D.SampleDesc.Quality = 0;
    desc2D.Usage = D3D11_USAGE_DEFAULT;
    desc2D.BindFlags = 0;
...
static void D3D11_DownloadFromTexture(
	...
	if (bufferStride == 0 || bufferImageHeight == 0)
	{
		...
		bufferImageHeight = textureRegion->h * SDL_GpuTextureFormatTexelBlockSize(textureSubresource->parent->format);
	}
...
static void D3D11_Blit(
    ...
    viewport.minDepth = destination->z;
    viewport.maxDepth = destination->z + destination->d;
    ...
    if (
        ...
        D3D11_PushFragmentShaderUniforms(
            driverData,
            commandBuffer,
            &source->textureSlice.layer,
            sizeof(Uint32)
    ...
    fragmentSamplerBinding.texture = source->textureSlice.texture;
    fragmentSamplerBinding.sampler =
    ...
    D3D11_EndRenderPass(driverData, commandBuffer);
}
...
static void D3D11_INTERNAL_InitBlitPipelines(
    ...
    shaderModuleCreateInfo.code = (Uint8*) fullscreenVertexShaderCode;
    shaderModuleCreateInfo.codeSize = SDL_strlen(fullscreenVertexShaderCode);
    ...
    if (renderer->fullscreenVertexShaderModule == NULL)
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to compile fullscreen vertex shader!");
    }

    /* Blit from 2D pixel shader */
    shaderModuleCreateInfo.code = (Uint8*) blitFrom2DPixelShaderCode;
    shaderModuleCreateInfo.codeSize = SDL_strlen(blitFrom2DPixelShaderCode);
    ...
    if (renderer->blitFrom2DPixelShaderModule == NULL)
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to compile blit from 2D pixel shader!");
    }

    /* Blit from 2D array pixel shader */
    shaderModuleCreateInfo.code = (Uint8*) blitFrom2DArrayPixelShaderCode;
    shaderModuleCreateInfo.codeSize = SDL_strlen(blitFrom2DArrayPixelShaderCode);
    shaderModuleCreateInfo.type = SDL_GPU_SHADERTYPE_FRAGMENT;
    ...
    if (renderer->blitFrom2DArrayPixelShaderModule == NULL)
    {
